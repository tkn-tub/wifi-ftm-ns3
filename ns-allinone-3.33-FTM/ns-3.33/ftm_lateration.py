...
def performTests():
	for i in range(0,100):
		...
		waf_command = ["--run=ftm-lateration", "--positions=" + str(positions), "--seed=" + str(seed), "--ftms=" + str(ftms),
...
#		subprocess.run(["./waf", waf_string])
		...
		bandwidth = 40
		waf_command[6] = "--bandwidth=" + str(bandwidth)
		waf_string = ' '.join(waf_command)
		#40MHz wired error
#		subprocess.run(["./waf", waf_string])

		error_model = "wireless"
		waf_command[5] = "--errorModel=" + error_model
...
def calculateResults():
	...
	for f in files:
		...
		for i in range(3, 11):
			distances_tmp = distances[:i]
			stations_tmp = stations[:i]
			xy = gps_solve(distances_tmp, stations_tmp)
			if round(np.linalg.norm(xy), 2) > outlier_threshold:
				#xy = np.array([1, 0])
				current_outliers.append([xy, [distances_tmp, stations_tmp]])
				#continue #remove points with very high distance
			output = [attributes[1][:-3], str(i), str(round(np.linalg.norm(xy), 2)), attributes[2][:-4], attributes[3].split('.')[0][:-4],
...
def visualize():
	...
	ax = sns.boxplot(x="positions", y="euclidean_error",
            hue="bandwidth", palette=["m", "g"],
            data=wired)
